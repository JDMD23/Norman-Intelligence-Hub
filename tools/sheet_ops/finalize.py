"""Finalize Lease Comps v4: rewrite the _Schema block to match the new layout,
then report QA. Run after migrate_structure.py and verify_wire.py both pass."""
from common import session, sheet_ids, batch_update, get_values, changelog, qa_status, SID

s = session()
schema_id = sheet_ids(s)['_Schema']
...
SPEC = [
    ...
    ('W', 'Latest Round Type', 'lr_type', 'text', 'calc', 'no', '', '=IF(OR($D{r}="",$V{r}=""),"",IFERROR(INDEX(SORT(FILTER({FundingRounds_Dates,FundingRounds_Types},FundingRounds_CompanyIds=$D{r}),1,FALSE),1,2),""))', f'Type of most recent round {W} -> Funding Rounds.'),
    ('X', 'Latest Round Amt ($M)', 'lr_amt', 'num', 'calc', 'no', '', '=IF(OR($D{r}="",$V{r}=""),"",IFERROR(LET(a,INDEX(SORT(FILTER({FundingRounds_Dates,FundingRounds_Amounts},FundingRounds_CompanyIds=$D{r}),1,FALSE),1,2),IF(a=0,"",a)),""))', f'Amount of most recent round {W} -> Funding Rounds.'),
    ('Y', 'Total Tracked Funding ($M)', 'total_fund', 'num', 'calc', 'no', '', "=IF($D{r}=\"\",\"\",IFERROR(LET(t,INDEX('Company Metrics'!$M:$M,MATCH($D{r},'Company Metrics'!$A:$A,0)),IF(t=\"\",\"\",t)),\"\"))", f'Sum of tracked rounds {W} -> Company Metrics. Semantic: tracked receipts, not researched narrative totals.'),
    ...
    ('AL', 'Lease-to-Total-Funding %', 'l2tf', 'pct', 'calc', 'no', '', '=IF(OR($AF{r}="",$Y{r}=""),"",$AF{r}/($Y{r}*1000000))', 'Projected Gross / wired total tracked funding.'),
...
# overwrite block, then delete surplus rows
