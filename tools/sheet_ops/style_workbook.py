...
# ---------- palette / type ----------
NAVY, SLATE, SLATE2, INK = '#0F172A', '#1E293B', '#334155', '#0F172A'
MUTED, RULE, PAPER = '#475569', '#CBD5E1', '#F8FAFC'
WIRE, CALC, GOV, WHITE = '#ECFEFF', '#F0FDF4', '#FFFBEB', '#FFFFFF'
GREEN_BG, GREEN_FG = '#DCFCE7', '#166534'
AMBER_BG, AMBER_FG = '#FEF3C7', '#92400E'
RED_BG, RED_FG = '#FEE2E2', '#991B1B'
GRAY_BG, GRAY_FG = '#F1F5F9', '#475569'
FONT = 'Roboto'
...
R += [{'mergeCells': {'range': rng(sid, 'A', 'B', 1, 1), 'mergeType': 'MERGE_ALL'}},
      ...
      fmt(rng(sid, 'A', 'B', 2, 2), bg=SLATE, fg='#CBD5E1', bold=False, size=10, v='MIDDLE'),
...
R += [sheet_props(sid, frozen_rows=0, tab='#166534', gridlines=False)]

# ---------- Companies (input) ----------
sid = ids['Companies']
R += header(sid, 'A', 'L') + body(sid, 'A', 'L', 1200)
R += [fmt(rng(sid, 'A', 'A', 2, 1200), fg=MUTED, bold=True),
      fmt(rng(sid, 'B', 'B', 2, 1200), bold=True),
      fmt(rng(sid, 'F', 'F', 2, 1200), num=('NUMBER', '0'), h='CENTER')]
R += zone_rule(sid, ['B', 'G'], 1200)
R += widths(sid, {'A': 88, 'B': 180, 'C': 230, 'D': 190, 'E': 230, 'F': 70, 'G': 110,
                  'H': 320, 'I': 260, 'J': 260, 'K': 260, 'L': 300})
R += [sheet_props(sid, frozen_rows=1, frozen_cols=2, tab=SLATE2)]
...
R += header(sid, 'A', 'O', bg='#166534') + body(sid, 'A', 'O', 1200, bg=CALC)
R += [fmt(rng(sid, 'A', 'A', 2, 1200), fg=MUTED, bold=True),
      fmt(rng(sid, 'B', 'B', 2, 1200), bold=True),
...
R += [sheet_props(sid, frozen_rows=1, frozen_cols=2, tab='#166534')]

# ---------- QA Harness (governance) ----------
sid = ids['QA Harness']
R += header(sid, 'A', 'F', bg='#92400E') + body(sid, 'A', 'F', 40)
...
R += [sheet_props(sid, frozen_rows=1, tab='#92400E')]

# ---------- Changelog (append-only receipts) ----------
sid = ids['Changelog']
R += header(sid, 'A', 'E', bg='#92400E') + body(sid, 'A', 'E', 1037)
...
sid = ids['_Schema']
R += header(sid, 'A', 'J', bg='#475569') + body(sid, 'A', 'J', 971)
...
R += [sheet_props(sid, frozen_rows=1, tab='#94A3B8')]

# ---------- Reference (vocabularies) ----------
sid = ids['Reference']
R += header(sid, 'A', 'M', bg='#475569') + body(sid, 'A', 'M', 1000)
R += [fmt(rng(sid, 'I', 'I', 1, 1), bg=WHITE), fmt(rng(sid, 'L', 'L', 1, 1), bg=WHITE)]
R += widths(sid, {c: 170 for c in 'ABCDEFGHJKM'}) + widths(sid, {'I': 24, 'L': 24})
R += [sheet_props(sid, frozen_rows=1, tab='#94A3B8')]
...
if 'LC_BACKUP_2026-09-02' in ids:
    R.append({'updateSheetProperties': {'properties': {'sheetId': ids['LC_BACKUP_2026-09-02'], 'hidden': True,
                                                        'tabColorStyle': {'rgbColor': hx('#94A3B8')}},
